...
#[tracing::instrument]
fn parse_seeds(input: &str) -> IResult<&str, (Vec<SeedMap>, Vec<Range<u64>>)> {
    let (input, seeds) = tag("seeds: ")
        .precedes(separated_list1(space1, separated_pair(complete::u64, tag(" "), complete::u64).map(|(x , y)| (x..x+y))))
        .parse(input)?;
